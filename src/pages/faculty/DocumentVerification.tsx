...
interface VerificationResult {
  ...
  data?: {
    ...
    parsedQr?: Record<string, string> | null;
    ocrText?: string;
    validatedBy?: string;
  };
}
      ...
      {verificationResult && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Card Info */}
            {verificationResult.data?.card && Object.keys(verificationResult.data.card).length > 0 && (
              <div>
                <h3 className="font-semibold mb-2">🆔 Extracted ID</h3>
                <ul className="list-disc list-inside text-sm">
                  {Object.entries(verificationResult.data.card).map(([key, value]) => (
            ...
            {/* QR Data */}
            {verificationResult.data?.parsedQr && (
              <div>
                <h3 className="font-semibold mb-2">📌 QR Data</h3>
                <pre className="bg-gray-100 p-2 rounded text-sm">{JSON.stringify(verificationResult.data.parsedQr, null, 2)}</pre>
              </div>
            )}
